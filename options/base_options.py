...
class BaseOptions():
    ...
    def initialize(self):
        ...
        #train_SDU_A_TA_CA_VER8 ==> proposed 
        #train_SDU_A_TA_CA_CNN ==> smooth dilated CNN ==> CNN 으로 바꾼거                 (Ablation 1)
        #train_SDU_A_ONLY_TA_CA ==> gated fusion 없앤거                                   (Ablation 2)
        #train_SDU_A_ONLY_CA_GF ==> Task Adaptor 빼고 channel-wise attention 만 적용한 것 (Ablation 3)
        #train_SDU_A_ONLY_CA ==> channel-wise attention 만 적용한 것                      (Ablation 4)      
        #train_SDU_A_ONLY_GATE ==> Task Adaptor 와 Task Channel-wise attention 없앤거     (Ablation 5)


        ## ablation도 따로 진행하기
        self.parser.add_argument('--name', type=str, default='train_SDU_B_ablation_TA5',
        ...
        self.parser.add_argument('--loadSize', type=int, default=224, help='scale images to this size')
        ...
        # 기존 downsample 4
        self.parser.add_argument('--n_downsample_global', type=int, default=2,
                                 help='number of downsampling layers in netG')
        # 기존 block 9
        self.parser.add_argument('--n_blocks_global', type=int, default=20,
        ...
        #추가옵션 (비교실험용)
        self.parser.add_argument('--USE_RB', type=int, default=True, help='RIGB 사용할 것인지 말것인지')
